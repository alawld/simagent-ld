...
|--------|---------|-------------|
| **Session** | Not playing vs playing vs terminal episode | **`SimAgentHarness`:** `reset`, `step`, **`runEpisode`**, **`loadSnapshot`** / **`getSerializedWorldState`**, **`setPaused`** / **`pause`** / **`resume`**. Terminal short-circuit when `GameOutcome` ≠ `None` or loaded snapshot already terminal. |
...
- **CLI:** `npm run sim:episode -- --seed 42 --max-ticks 2000 [--scenario-id ID] [--opponent ai|none] [--policy noop|heuristic] [--commands-file PATH] [--ld-experiment K] [--ld-variation K] [--ld-iteration K]` prints **one JSON line** per episode to stdout (**NDJSON** with **`--repeat N`** or **`--seeds a,b,c`**). **`--policy heuristic`** toggles player `SetBehaviorRatio` on a fixed cadence (baseline smoke). **`--commands-file`** is a JSONL file: each **non-empty line** is a JSON **array** of `AgentSimCommand` (omit `issuedAtTick`); line `min(tickIndex, lastLine)` repeats the last line after EOF. Do not combine `--commands-file` with a non-`noop` **`--policy`**. For custom agents, call **`runEpisode({ getCommands })`** from TypeScript.
- **Bridge guide (LD AI Config / experimentation):** **`docs/sim-agent-launchdarkly-bridge.md`** — orchestrator loop, metric paths, fairness (**`seed`**), **`episode-metrics-catalog.ts`** for registerable field names.
- **Human wiring checklist (secrets, LD, orchestrator — outside this repo):** **`docs/Human-to-do.md`**.
...
**LaunchDarkly usage (outside this repo)** — see **`docs/sim-agent-launchdarkly-bridge.md`**.

1. Define **custom metrics** in LD that match fields you care about (e.g. `metrics.victory`, `metrics.finalTick`, `metrics.playerFoodTotal`, `wallClockMs`); use **`SIM_AGENT_METRICS_DOT_PATHS`** in **`episode-metrics-catalog.ts`** as a checklist.
...
| Phase | State | Notes |
|-------|--------|--------|
| **A — Harness** | **Landed** | `SimAgentHarness`: `reset`, `step`, **`runEpisode`**, **`loadSnapshot`**, **`getSerializedWorldState`**, **`pause`/`resume`**, `getObservation`, … |
...
**Quick links:** `src/sim-agent/harness.ts` · `src/sim-agent/types.ts` · `src/sim-agent/jsonl-session.ts` · `src/sim-agent/observation-channels.ts` · `src/sim-agent/session-recording.ts` · `src/sim-agent/replay-input-log.ts` · `src/sim/command-applicability.ts` · `docs/sim-agent-commands.md` · `docs/sim-agent-mdp.md` · `docs/sim-agent-launchdarkly-bridge.md` · `docs/Human-to-do.md` · `src/sim-agent/episode-metrics.ts` · `src/sim-agent/episode-metrics-catalog.ts` · `src/sim-agent/policies.ts` · `src/sim-agent/scenario-thresholds.ts` · `src/sim-agent/eval-grid.ts` · `src/sim/training-scenarios.ts` · tests · `scripts/run-agent-episode.ts` · `scripts/export-agent-session.ts` · `scripts/sim-agent-jsonl.ts` · `scripts/run-eval-grid.ts`

**Follow-ups:** Implement the **orchestrator** (LD SDK + `track` + `sim:episode` / NDJSON) using **`docs/sim-agent-launchdarkly-bridge.md`**; more **§5** factories; tighten **thresholds** per scenario as design locks; **`scripted` opponent** + HTTP transport (Phase A); `export` from `src/main.ts` if needed.
